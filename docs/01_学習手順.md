...
各コンテナのメモリ使用量（`.env` のデフォルト設定）:

| コンテナ | mem_limit | JVM ヒープ | 実質使用量 |
|---|---|---|---|
| Elasticsearch | 2GB | 1GB | ~1.5-2GB |
| Kibana | 1GB | - | ~0.5-0.8GB |
| Logstash | 1GB | 512MB | ~0.5-0.8GB |
| ElastAlert2 | 制限なし | - | ~0.1-0.2GB |
| Python ML | 制限なし | - | ~0.1-0.3GB |
| **合計** | | | **~3-4GB** |

これに加えて Windows 本体が ~2.5GB、Docker Desktop / WSL2 が ~0.5GB 使用するため、**16GB PC ならデフォルト設定で快適に動作**する。

**8GB PC の場合**は `.env` を以下のように変更する:
...
> **8GB PC での制約**: ML ジョブの実行が遅くなる可能性がある。Kibana と他のアプリケーションの同時使用は控えめにすること。
