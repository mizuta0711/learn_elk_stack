...
1. 左メニュー → **Machine Learning**
2. **Anomaly Detection** → **Create job**
3. data view: `sensor-data` を選択
4. **Multi-metric job** を選択（複数センサーを同時に分析）
5. 設定:
   - **Bucket span**: `15m`（15分間隔で分析）
   - **Detectors**:
     - `mean(vibration)` を追加
     - `mean(temperature)` を追加
     - `mean(current)` を追加
   - **Split field（Influencers）**: `device_id`
6. **Job ID** に `sensor-anomaly-detection` と入力
7. **Create job** をクリック

### 3-2. ジョブ実行

ジョブ作成後、過去データに対して自動的に分析が実行される。
処理が完了するまで数分待つ。

### 3-3. 結果の確認

1. **Anomaly Explorer** タブをクリック
2. 時間範囲を `2026-01-01 ～ 2026-03-31` に設定
3. 以下を確認:
...
### 3-4. Anomaly Detection の結果をダッシュボードに追加

1. Anomaly Swimlane の画面で **Add to dashboard** をクリック（または後のStep 5でダッシュボードに組み込む）
...
1. 左メニュー → **Dashboard** → **Create dashboard**
2. **Create visualization** をクリック

### 5-2. センサー値の時系列グラフ

1. Visualization type: **Line**
2. Data view: `sensor-data`
3. Horizontal axis: `@timestamp`
4. Vertical axis: `Average of vibration`
5. **Break down by**: `device_id`（Top 3）
6. 右上の **Save and return** をクリック

同様に `temperature`, `current` のグラフも追加する。

### 5-3. 閾値ラインの追加

1. 作成した vibration のグラフを編集
2. **Reference line** を追加
   - Value: `8.0`（vibration の上限閾値）
   - Color: 赤
   - Label: `上限閾値`
3. Save and return
...
2. Data view: `prediction-results`（Step 4を実行した場合）
3. Metric: `Unique count of device_id`
4. フィルタで `alert_level: normal` を設定
5. タイトル: `正常`
6. Save and return

同様に `caution`, `warning`, `danger` の Metric も追加する。

### 5-5. Anomaly Swimlane の追加

1. **Create visualization**
2. Visualization type: **Anomaly swim lane**
3. ML job: `sensor-anomaly-detection`（Step 3で作成）
4. View by: `device_id`
5. Save and return
...
Step 4 で推論結果を書き込んだ後、`anomaly_score >= 0.7` のデータがあれば、ElastAlert2 がアラートを検知してログに出力する。

現在の設定は `debug` モード（ログ出力のみ）。
メールやWebhook通知を試す場合は、`elastalert/rules/anomaly-alert.yaml` を編集する。

### 6-2. Kibana のアラート機能（別のアプローチ）
...
- [ ] Step 3: Elasticsearch ML でジョブを作成し、Anomaly Swimlane で結果を確認できた
- [ ] Step 3: 故障日の前にセンサー異常が検知されることを確認できた
